            ...
            <nav className="hidden md:flex items-center space-x-8">
              {NAV_ITEMS.map((item) => (
                <button
                  ...
                  className="text-sm font-medium text-gray-400 hover:text-primary-400 transition-colors focus:outline-none"
                >
                  {item.label}
                </button>
              ))}
            ...
            <div className="px-4 pt-2 pb-6 space-y-1">
              {NAV_ITEMS.map((item) => (
                <button
                  key={item.label}
                  onClick={() => handleNavClick(item.view)}
                  className="block w-full text-left px-3 py-3 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-gray-800 transition-colors"
                >
                  {item.label}
                </button>
              ))}
